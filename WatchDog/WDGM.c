...
void WDGM_MainFunction(void)
{
    ...
    {
        if (led_no_calls >= 8 && led_no_calls <= 12) { // check no. of correct calls is between 8 and 12 or not
            status = OK;
        } else {
            status = NOK;
        }
...
{
    led_correct_calls++; // increment correct calls counter each time LEDM_Manage is called this
}
